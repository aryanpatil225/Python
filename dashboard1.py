...
def submit_ticket():
    ...
    name = name_entry.get()
    event_name = event_name_entry.get().upper()  # Convert event name to uppercase
    ...
    # Display submitted information in the result frame
    result_frame.config(text=f"Ticket Information\n{'-' * 20}\nEvent Name: {event_name}\nName: {name}\nOrganizer Name: {organizer_name}\nOrganizer Contact: {organizer_contact}\nOrganizer Email: {organizer_email}\nDate: {date}\nTime: {time}\nLocation: {location}\nAge: {age}\nGame: {game}")
...
result_frame = tk.LabelFrame(root, text="Ticket Information", padx=10, pady=10)
result_frame.grid(row=11, column=0, columnspan=2, padx=10, pady=10, sticky="we")
...
window_width = 500
window_height = 750  # Increased height to accommodate the result frame
